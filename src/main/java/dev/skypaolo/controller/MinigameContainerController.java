...
public class MinigameContainerController implements Initializable {
    ...
    private void loadMinigameIcon(String imagePath) {
        try {
            ...
            System.err.println(
                "[Vault-Tec Warning] Could not load minigame icon: " + imagePath
            );
        }
    }
    ...
    public void handleBackToHub(ActionEvent event) {
        ...
        try {
            ...
            System.err.println(
                "[Vault-Tec Critical] Failed to return to hub: " +
                    e.getMessage()
            );
            e.printStackTrace();
        }
    ...
    private void loadGameContent() {
        ...
        try {
            ...
            System.err.println(
                "[VT-OS CRITICAL] Failed to load minigame: " + e.getMessage()
            );
            e.printStackTrace();
            showErrorContent();
        }
    ...
    private void loadPlusOuMoinsGame() throws IOException {
        ...
        System.out.println(
            "[VT-OS] Plus ou Moins simulation loaded successfully."
        );
    }
    ...
    private void showPlaceholderContent() {
        ...
        startedLabel.setStyle(
            "-fx-font-size: 24px; -fx-font-weight: bold; -fx-text-fill: #FFE600;"
        );

        Label minigameNameLabel = new Label(currentMinigame.getTitle());
        minigameNameLabel.setStyle(
            "-fx-font-size: 18px; -fx-text-fill: #3C8DBC;"
        );

        Label placeholderLabel = new Label(
            "Cette simulation n'est pas encore implémentée.\nVault-Tec travaille dur pour vous offrir la meilleure expérience!"
        );
        placeholderLabel.setStyle(
            "-fx-font-size: 14px; -fx-text-fill: #AAAAAA;"
        );
        placeholderLabel.setTextAlignment(
        ...
        stopButton.setStyle(
            "-fx-background-color: #DC3545; -fx-text-fill: white; -fx-font-weight: bold; -fx-padding: 10 20;"
        );
    ...
    private void showErrorContent() {
        ...
        errorLabel.setStyle(
            "-fx-font-size: 24px; -fx-font-weight: bold; -fx-text-fill: #DC3545;"
        );
        ...
        backButton.setStyle(
            "-fx-background-color: #6C757D; -fx-text-fill: white; -fx-font-weight: bold; -fx-padding: 10 20;"
        );
